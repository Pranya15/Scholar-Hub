...
# Or use the newer Supabase publishable key instead:
...
2. Run `supabase/schema.sql` to create the tables, triggers, storage bucket, and RLS policies used by the current application.
3. Run `supabase/seed.sql` if you want the included scholarship records.

The file under `supabase/migrations/` belongs to a newer, incompatible schema variant and should not be run for this application version. The app currently uses the column names and storage bucket defined in `schema.sql`.
