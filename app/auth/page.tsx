...
function AuthContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isSupabaseConfigured()) {
      setErrorMessage(
        "Supabase is not configured. Set NEXT_PUBLIC_SUPABASE_URL and either NEXT_PUBLIC_SUPABASE_ANON_KEY or NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY in .env or .env.local, then restart the development server."
      );
      setIsLoading(false);
      return;
    }
    ...
    try {
      if (mode === "login") {
        ...
        if (error) {
          ...
          if (error.message.includes("User already registered") || error.message.includes("unique constraint")) {
            ...
            userMsg = "Supabase has temporarily limited new registration emails. Wait a few minutes, then try again with a single request. You can also disable email confirmation in Supabase Auth settings for local testing.";
          }
          setErrorMessage(userMsg);
          setIsLoading(false);
          setIsRegisterSuccess(false);
      ...
      let errMsg = err.message || "An unexpected error occurred during authentication.";
      if (errMsg.includes("Failed to fetch") || errMsg.includes("NetworkError")) {
        errMsg = "Failed to connect to the database API. If you are in production (Vercel), please make sure NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY are correctly configured in your Vercel Environment Variables.";
      }
