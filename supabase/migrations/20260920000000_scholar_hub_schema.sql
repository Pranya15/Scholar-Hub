-- Canonical schema for the deployed Scholar Hub application.
-- Keep this migration aligned with supabase/schema.sql.
...
CREATE TRIGGER tr_profiles_role_lock BEFORE UPDATE ON public.profiles
...
CREATE TRIGGER tr_applications_tracking_no BEFORE INSERT ON public.applications
...
WITH CHECK (public.is_officer_or_admin() OR auth.uid() = user_id);
...
FOR SELECT TO authenticated USING (
  bucket_id = 'documents' AND ((storage.foldername(name))[1] = auth.uid()::text OR public.is_officer_or_admin())
);
